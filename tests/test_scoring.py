...
def test_named_punt_strategy_works():
    df = pd.DataFrame(
        {
            "player": ["A", "B", "C"],
            "pts": [20, 10, 5],
    ...
    balanced = add_9cat_scores(df, punt_strategy="balanced")
    punt_ft = add_9cat_scores(df, punt_strategy="punt_ft")

    assert "total_9cat_z" in balanced.columns
    assert "total_9cat_z" in punt_ft.columns
    assert not balanced["total_9cat_z"].equals(punt_ft["total_9cat_z"])